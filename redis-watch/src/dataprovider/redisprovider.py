...
class RedisStatsProvider(object):
    ...
    def get_memory_info(self, server, from_date, to_date):
        ...
        for row in rows:
            ...
            timestamp = datetime.fromtimestamp(int(row['timestamp'][0:10]))
    ...
    def get_command_stats(self, server, from_date, to_date, group_by):
        ...
        for x in xrange(0,len(counts)):
            ...
            # convert the timestamp
            timestamp = int(time_stamps[x][0:10])
    ...
    def get_top_counts(self, server, from_date, to_date, seconds_key_name,
        ...
        if diff.days > 2 :
            ...
            # print str(int(str(start))),from_date_end_epoch
            for x in range(int(str(start)[:10]), int(str(from_date_end_epoch)[:10]) + 1):
            ...
            for x in range(int(str(to_date_begin_epoch)[:10]), int(str(end)[:10]) + 1):
